...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM series JOIN books ON books.series_id = series.id JOIN character_books where character_books.book_id = books.id JOIN characters ON characters.id = character_books.character_id GROUP BY character.species ORDER BY COUNT(characters.species) DESC LIMIT 1 WHERE characters.species = 'human' "
end
